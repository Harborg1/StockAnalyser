...
# Seed 42 results...
# > Gen 1 Lowest Error: 0.3265
# > Gen 2 Lowest Error: 0.2857
#   Gen 3 Lowest Error: 0.2041
#   Gen 4 Lowest Error: 0.2857

#Date: 2021-11-19 | Best: $15,286 | Market: $21,935 | Worst: $20,326

# Seed 43 results...
#  Gen 1 Lowest Error: 0.2041
#  Gen 2 Lowest Error: 0.2857
#  Gen 3 Lowest Error: 0.2041
#  Gen 4 Lowest Error: 0.2449

#Date: 2021-11-19 | Best: $14,764 | Market: $21,935 | Worst: $29,003

# Seed 44 results...
#Gen 1 Lowest Error: 0.3265
#Gen 2 Lowest Error: 0.285
#Gen 3 Lowest Error: 0.2857
#Gen 4 Lowest Error: 0.2857
#Date: 2021-11-19 | Best: $14,907 | Market: $21,935 | Worst: $30,713

#Seed 45 results...
#   > Gen 1 Lowest Error: 0.3265
#   > Gen 2 Lowest Error: 0.3265
#   > Gen 3 Lowest Error: 0.2857
#   > Gen 4 Lowest Error: 0.3265
#Date: 2021-11-19 | Best: $18,109 | Market: $21,935 | Worst: $26,744
...
class GeneticOptimizer:
    ...
    def _evaluate_gene_worker(gene, current_idx, features, returns, tickers, seed):
        ...
        if len(X) < 50: return 999.0 # High error for invalid genes
        ...
        # --- VALIDATION PHASE (ACCURACY BASED) ---
        X_val, y_val = [], []
        # Using a small validation slice immediately following training data
        val_start = train_end
        val_end = current_idx

        # If validation window is empty (edge case), use last chunk of training
        if val_start >= val_end:
             val_start = train_end - HOLDING_PERIOD
             val_end = train_end

        for t in range(val_start, val_end, HOLDING_PERIOD):
             f_perf = returns.iloc[t : t + HOLDING_PERIOD].sum()
             med = f_perf.median()
             for s in tickers:
                s_slice = t - window
                ...
                if len(feat) == window * NUM_FEATURES:
                    X_val.append(feat)
                    y_val.append(1.0 if f_perf[s] > med else 0.0)

        if not X_val: return 999.0

        preds = engine.predict(np.array(X_val))
        
        # Calculate Prediction Error (MSE between Prediction Score and Actual Class Label)
        # Note: RankNet outputs raw scores, not probabilities. 
        # Ideally, we check if the ranking is correct. 
        # Simple Proxy: Check if high scores align with Class 1 (y=1)
        
        # Here we use Binary Cross Entropy / Accuracy proxy or simple Classification Error
        # Convert scores to binary predictions (Top 50% vs Bottom 50%)
        
        pred_series = pd.Series(preds)
        median_score = pred_series.median()
        binary_preds = (pred_series > median_score).astype(float).values
        
        # Error = Fraction of incorrect classifications (Accuracy inverted)
        correct = (binary_preds == np.array(y_val)).sum()
        total = len(y_val)
    ...
    def optimize(self, current_idx):
        set_seed(SEED)
        print(f"  [GA] Evolving parameters for index {current_idx} (Minimizing Error)...")
        ...
        for g in range(self.generations):
            ...
            with concurrent.futures.ProcessPoolExecutor() as executor:
                ...
                for i, gene in enumerate(population):
                    worker_seed = SEED + (current_idx * 10000) + (g * 1000) + i
                    fut = executor.submit(
                        GeneticOptimizer._evaluate_gene_worker,
                        gene, current_idx, self.features, self.returns, self.tickers, worker_seed
                    )
            ...
            # --- CHANGE: SORT ASCENDING (LOWER ERROR IS BETTER) ---
            scored = sorted(results_by_index, key=lambda x: x[0], reverse=False)

            print(f"    > Gen {g+1} Lowest Error: {scored[0][0]:.4f}")
...
if __name__ == "__main__":
    ...
    print("\n--- Phase 1: Running Genetic Optimization (Minimizing Error) ---")
    ...
    plt.title('Final Backtest (Minimizing Prediction Error)')
    plt.ylabel('Portfolio Value ($)')
    path = os.path.join("results", f'Seed {SEED} results_v2.png')
    plt.savefig(path)
